...
// MediaPipe hand poses detection (see also https://github.com/opencv/opencv_zoo/tree/master/models/handpose_estimation_mediapipe)
namespace OpenCVForUnityExample.DnnModel
{
    public class MediaPipeHandPoseEstimator
    {
        ...
        public MediaPipeHandPoseEstimator(string modelFilepath, float confThreshold = 0.8f, int backend = Dnn.DNN_BACKEND_OPENCV, int target = Dnn.DNN_TARGET_CPU)
        {
            ...
            {
                handposeEstimationNet = Dnn.readNet(modelFilepath);
            }
            ...
            handposeEstimationNet.setPreferableBackend(this.backend);
            handposeEstimationNet.setPreferableTarget(this.target);
        }
        ...
        public virtual Mat Infer(Mat image, Mat palm)
        {
            ...
            // Forward
            handposeEstimationNet.setInput(input_blob);
            List<Mat> output_blob = new List<Mat>();
            handposeEstimationNet.forward(output_blob, handposeEstimationNet.getUnconnectedOutLayersNames());
        ...
        {
            if (handposeEstimationNet != null)
                handposeEstimationNet.Dispose();
